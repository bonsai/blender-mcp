...
class SessionLogger:
    ...
    def cleanup(self):
        ...
        try:
            # ログファイルを削除
            for log_file in [self.command_log_file, self.blender_log_file, self.state_file, self.design_file]:
                if log_file.exists():
                    log_file.unlink()
            ...
            logger.info(f"Logs cleaned up for {self.session_id}")
        except Exception as e:
...
if __name__ == "__main__":
    # テスト用
    logger_instance = SessionLogger("test-session-001")
    ...
    # Blender操作ログ
    logger_instance.log_blender_operation(
        "create_cube",
    ...
    # 状態保存
    logger_instance.save_state({
        "objects": ["Cube"],
        "selection": "Cube"
    })
    
    # ドキュメント生成
    logger_instance.generate_design_doc()
    
    # サマリー表示
    print(json.dumps(logger_instance.get_log_summary(), indent=2))
