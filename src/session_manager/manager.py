"""
Session Manager V2 - ログシステム・永続化統合版
"""
...
try:
    ...
    from .persistence import SessionPersistence, get_persistence
except ImportError:
    from logger import SessionLogger
    from persistence import SessionPersistence, get_persistence
...
class SessionManager:
    ...
    def restore_session(self, session_id: str) -> bool:
        ...
        try:
            ...
            if not session_data:
                logger.warning(f"Session not found in persistence: {session_id}")
                return False
            ...
            if "state" in session_data:
                ...
                session.state.objects = state.get("objects", [])
                session.state.selection = state.get("selection")
                session.state.last_command = state.get("last_command")
